...
use scalar_index_abi::{
    InMemoryArrowStream, IndexBuildProgress, IndexStore, OpTrain, Registry, Result,
};
...
#[tokio::test(flavor = "multi_thread")]
async fn python_package_exposes_plugin_and_provider_registers_it() -> Result<()> {
    let package_root = build_python_plugin_package();
    let script =
        workspace_root().join("examples/scalar-index/plugin/tests/python/check_package.py");
...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(3)
        .expect("plugin package lives under workspace/examples/scalar-index/plugin")
        .to_path_buf()
}
